...
import type { BggSearchItem } from "../domain/types";
import { CreatePlayForm } from "./CreatePlayForm";
import type { CreatePlayFormData, PlayerEntry } from "../domain/types";
...
/** "" = show select, "other" = show BGG search input, number = show selected my-game card */
type GameChoice = "" | "other" | number;
...
export function CreatePlayPage({
	...
	const handleSelectMyGame = useCallback((item: MyGameItem) => {
		setForm((prev) => ({ ...prev, game: item.name }));
		setSelectedBggId(item.bggId);
		setBggImageUrl(item.thumbnailUrl ?? item.imageUrl);
		setGameChoice(item.bggId);
		setBggResults([]);
	}, []);
	...
	const handleClearBggSelection = useCallback(() => {
		setSelectedBggId(undefined);
		setBggImageUrl(undefined);
		if (typeof gameChoice === "number") setGameChoice("");
	}, [gameChoice]);
	...
	return (
		<CreatePlayForm
			...
			onSelectMyGame={handleSelectMyGame}
